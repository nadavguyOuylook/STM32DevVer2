...
void LCD_1IN8_Display(UWORD *Image)
{
	...
	for(i=0;i<LCD_1IN8_HEIGHT;i++)
	{
		...
		{
			//LCD_1IN8_WriteData_NLen16Bit(Image[i*LCD_1IN8_WIDTH+j],1);
			DEV_SPI_WRITE( (uint8_t)(Image[i*LCD_1IN8_WIDTH+j] & 0XFF) );
			DEV_SPI_WRITE( (uint8_t)(Image[i*LCD_1IN8_WIDTH+j] >> 8) );
...
void LCD_1IN8_DisplayWindows(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend, UWORD *Image)
{
	...
	for(i=0;i<Yend-Ystart;i++)
	{
		...
		{
			//LCD_1IN8_WriteData_NLen16Bit(Image[i*LCD_1IN8_WIDTH+j],1);
			DEV_SPI_WRITE( (uint8_t)(Image[i*(Xend-Xstart)+j] & 0XFF) );
			DEV_SPI_WRITE( (uint8_t)(Image[i*(Xend-Xstart)+j] >> 8) );
